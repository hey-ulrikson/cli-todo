...
- Note: one short phrase of context. Name the author with a bare first name (`Alice`) **only when the work is someone else's**. The user's own work gets no tag — **never write `Own`**; an untagged task is theirs by default. Drop `Author:` prefix and role labels (`(intern)`, `(own)`).
- **No trailing period on the note** — it's a phrase, not a sentence. `Alice`, not `Alice.`
- Don't restate what `status` already encodes: `waiting` ⇒ blocked (don't say "waiting on …"); `review` ⇒ you owe a review (don't say "review" in the note — but `Re-review` is worth keeping as it adds information).
- Push deadlines into `due_at`, not the note. Strip year from dates when current.
- One phrase. No ellipsis dumps. Prefer dropping non-load-bearing detail over abbreviating to opacity.

Examples:
- ✅ `PR 521 Review — Alice, re-review`
- ✅ `PR 495 Review — feature rollout`
- ✅ `Fix billing issues — TASK-710`
- ❌ `Review PR 521 — Alice. Re-review.` (PR number should lead; no trailing period)
...
- **Always pass `-cmd '.timeout 5000'`** to sqlite3 (e.g. `sqlite3 -cmd '.timeout 5000' "<DB path>" "<SQL>"`). A second todo command may be writing concurrently; without a busy timeout the write fails instantly with "database is locked".
...
2. Run the query or mutation directly via `sqlite3 -cmd '.timeout 5000' "<DB path>" "<SQL>"` (or heredoc for multi-statement).
